...
export class GetArrayValuePipe implements PipeTransform {
  ...
  transform(value: any, Columnname: any): any {
    if (Columnname.field.split(".").length == 1) {
      value[Columnname.field] = this.Formatdata(value[Columnname.field], Columnname["type"], Columnname["format"]);
      return value[Columnname.field];
    }
    else if (Columnname.field.split(".").length == 2) {
      let data = value[Columnname.field.split(".")[0]];
      if (data != null) {
        data[Columnname.field.split(".")[1]] = this.Formatdata(data[Columnname.field.split(".")[1]], Columnname["type"], Columnname["format"]);
        return data[Columnname.field.split(".")[1]];
      }
    }
    else {
      let data = value[Columnname.field.split(".")[0]];
      data = data[Columnname.field.split(".")[1]];
      data[Columnname.field.split(".")[2]] = this.Formatdata(data[Columnname.field.split(".")[2]], Columnname["type"], Columnname["format"]);
      return data[Columnname.field.split(".")[2]];
    }
  }
